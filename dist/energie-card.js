  ...
  render() {
    ...
    const schemas = [
      ...
      [ 
        ...
        { name: "cap_st", label: "Capacité 1 StorCube (Wh)", selector: { number: { min: 0, max: 2000, mode: "box" } } },
...
class EnergieCard extends LitElement {
  ...
  render() {
    ...
    // Capacités
    const capST = parseFloat(c.cap_st) || 1024;
    const capMV = parseFloat(c.cap_mv) || 5120;
    const totalCap = (capST * 2) + capMV;

    // SOC Global Pondéré (Wh totaux / Capacité totale)
    const currentWh = (s1/100 * capST) + (s2/100 * capST) + (s3/100 * capMV);
    const globalSoc = Math.round((currentWh / totalCap) * 100);
    ...
    return html`
      <ha-card style="border-color: ${cardStatusColor}66">
        <div class="card-header">
          <span class="title">${isNight ? '🌙 VEILLE' : (c.title || 'ENERGIE')}</span>
        ...
        <div class="main-stats">
          ...
          <div class="stat-box" title="ST1: ${s1}% | ST2: ${s2}% | MV: ${s3}%">
